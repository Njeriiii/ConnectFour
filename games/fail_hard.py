...
class ConnectFourFailHard:
    def __init__(self):
        self.board = self.create_board()

    def create_board(self):
        print("Beginning FAil-hard alpha-beta pruning")
        ...
        if is_maximizing:  # Whether it's the AI's turn to maximize the score.
            ...
            for col in range(7):
                if self.is_valid_move(col):
                    ...
                    if beta <= best_score:  # if score > beta: break
                        break
                    alpha = max(alpha, best_score)  # Fail-hard alpha-beta pruning
            ...
            for col in range(7):
                if self.is_valid_move(col):
                    ...
                    beta = min(beta, score)  # Fail-hard alpha-beta pruning
            return best_score
